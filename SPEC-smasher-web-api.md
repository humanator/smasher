...
- [ ] JSON/SSE endpoints cover: submit pipeline, live event stream, list/status, human-gate answer.
- [ ] Static file route serves `smasher-spa`'s build output with SPA-style fallback routing.
- [ ] `smasher serve` behavior is otherwise unchanged from the user's perspective (same command, same port).
- [ ] askama templates and HTMX JS removed.
- [ ] All new/changed code has unit + integration coverage; `cargo test -p smasher-web` and `cargo clippy -p smasher-web` clean.
