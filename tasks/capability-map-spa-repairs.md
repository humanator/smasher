...
| `candidate-preview` | #9: screenshot thumbnail → full-size, interactive lightbox of the live bundle with a ⟲ reset; params section (`generation_params`); captured-at on failed cards; in both `CandidateCard` and `GalleryGate`. Scorecard badges, checkbox and comment box stay on the card | `spa-shell` | Done 2026-09-25 |
...
| `workflow-detail` | New `/workflows/{id}` route: name, source dir, Edit button, Run button (opens the `run-launch` dialog), active-run summary card linking to `/runs/{id}`, and that workflow's run history (runs filtered by `workflow_id`). Catalog links to it | `run-launch`, `run-summary` | Done 2026-09-25 |
