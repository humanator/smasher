...
### Phase A: `run-summary`
- [x] Task 2: `lib/runStatus.ts` and `lib/graphError.ts`
- [x] Task 3: `RunMetaList` in `RunDetail`: a failed run shows its error
- [x] Task 4: Graph polls while running, stops after a final fetch, shows errors inline and fits the width
- [x] Task 5: `TokenCounter` total, 3s poll and `active` prop
- [x] Task 6: `RunList` shows `unnamed`, and `graph_name` is typed `string | null`
- [x] Task 7: `e2e/run-summary.spec.ts`

### Checkpoint A: `run-summary`
- [x] Full Vitest suite, `check`, `lint` and the touched e2e specs are green. Mark the module done in the map. Review with Jobsworth. (Review waived: Jobsworth chose to run straight through.)

### Phase B: `question-card`
- [x] Task 8: Kind badge, question id, first fetch straight away, empty state, store reset
- [x] Task 9: Free-text Submit button: disabled when blank, keeps the text on failure, blocks double submits
- [x] Task 10: Multiple choice as radio buttons with Submit; approval buttons block double submits
- [x] Task 11: `e2e/question-card.spec.ts`

### Checkpoint B: `question-card`
- [x] Green as above. Mark the module done in the map. Review with Jobsworth. (Review waived: Jobsworth chose to run straight through.)

### Phase C: `event-log`
- [x] Task 12: `lib/eventFormat.ts` for all 17 kinds
- [x] Task 13: Events store sequence keys, full-JSON de-duplication, and clearing on run change
- [x] Task 14: `EventLog` renders newest-first, colour-coded, indented and faded
- [x] Task 15: The log follows new events
- [x] Task 16: `e2e/event-log.spec.ts`

### Checkpoint C: `event-log`
- [x] Green as above. Mark the module done in the map. Review with Jobsworth. (Review waived: Jobsworth chose to run straight through.)

### Phase D: `workflow-detail`
- [x] Task 17: Pull `RunTable` out of `RunList`, with no behaviour change
- [x] Task 18: Page-header title slot, the `/workflows/{id}` route, and a page header that handles not-found
- [x] Task 19: Run card (`pickFeaturedRun`) and run history
- [x] Task 20: Catalog link, `e2e/workflow-detail.spec.ts`, and backlog follow-ups
