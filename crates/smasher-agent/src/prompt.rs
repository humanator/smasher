...
pub const DESIGN_FACTORY_CONVENTIONS: &str = "\n\nThis pipeline step has a shared component kit available at ./design-kit/ — a filesystem path, relative to your current working directory, for use with your file-reading tools only. Read ./design-kit/README.md and open ./design-kit/catalog.html on disk before writing any UI markup. Compose screens from its button/input/list-row/drawer/modal components and the tokens in ./design-kit/tokens.css (plus the color/font/radius/shadow tokens it documents from style.css) — never hand-roll a new component or a raw color, spacing, radius, or shadow value. That filesystem path is separate from the URL a browser uses to load the kit: inside candidate HTML, every <link>/<script> tag that references the kit must use the absolute URL path /design-kit/... (e.g. <link rel=\"stylesheet\" href=\"/design-kit/tokens.css\">) — never a relative path like ../../design-kit/... or ./design-kit/.... The candidate is served from a different directory depth at render time than in the dashboard later, so only an absolute URL path resolves correctly in both places; this rule applies only to hrefs/srcs written into candidate HTML, never to paths you pass to your own file-reading tools. design-kit/tokens.css and components.css do NOT define the color/font/radius/shadow tokens themselves (only spacing/overlay/focus-ring) — those live in /static/style.css, which every candidate's <head> must also link (<link rel=\"stylesheet\" href=\"/static/style.css\">), before the design-kit stylesheets. Without it, every var(--token) the kit and your own CSS reference resolves to nothing and the page renders with default black-on-transparent text. If the task asks you to build or edit a candidate under a given directory, that directory's entry point must be named index.html exactly, and must stay at exactly that name — the render/lint/critique tools that run next open only that file, by that name.";
