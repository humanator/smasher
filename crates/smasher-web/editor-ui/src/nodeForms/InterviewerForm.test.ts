...
import InterviewerForm from './InterviewerForm.svelte';
...
describe('InterviewerForm', () => {
  it('pre-populates question from attrs', () => {
    ...
    expect(screen.getByTestId('interviewer-question')).toHaveValue('pick a direction');
  });
  ...
  describe('gallery mode', () => {
    it('defaults the toggle off and hides candidate_count when attrs.gallery is absent', () => {
      ...
      expect(screen.getByTestId('interviewer-gallery-toggle')).not.toBeChecked();
      expect(screen.queryByTestId('interviewer-candidate-count')).not.toBeInTheDocument();
    });
    ...
    it('defaults the toggle on and shows candidate_count when attrs.gallery is true', () => {
      ...
      expect(screen.getByTestId('interviewer-candidate-count')).toHaveValue('3');
    });
    ...
    it('disables the gallery toggle when Answer mode is not Free-form', () => {
      ...
      expect(screen.getByTestId('interviewer-gallery-toggle')).toBeDisabled();
    });
  });
  ...
  describe('answer mode', () => {
    it('defaults to Free-form when neither approve nor options is set', () => {
      ...
      expect(screen.getByTestId('interviewer-answer-mode')).toHaveValue('freeform');
      expect(screen.queryByTestId('interviewer-options')).not.toBeInTheDocument();
    });

    it('defaults to Yes/No when attrs.approve is true', () => {
      render(InterviewerForm, { props: { attrs: { approve: true }, onChange: vi.fn() } });

      expect(screen.getByTestId('interviewer-answer-mode')).toHaveValue('approve');
    });
    ...
    it('defaults to Options and pre-populates the options field when attrs.options is a string', () => {
      ...
      expect(screen.getByTestId('interviewer-answer-mode')).toHaveValue('options');
      expect(screen.getByTestId('interviewer-options')).toHaveValue('proceed, iterate');
    });
    ...
    it('disables the answer-mode select while gallery mode is on', () => {
      ...
      expect(screen.getByTestId('interviewer-answer-mode')).toBeDisabled();
    });
  });
});
