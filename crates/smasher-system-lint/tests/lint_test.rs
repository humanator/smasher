...
use smasher_system_lint::{lint, LintError};

#[test]
fn clean_candidate_passes_both_checks() {
    let report = lint(Path::new("fixtures/clean-candidate")).unwrap();
    assert!(report.passed(), "expected clean-candidate to pass: {report:?}");
}
...
fn violating_candidate_fails_both_checks_with_expected_violations() {
    ...
    assert!(token_check.violations.iter().any(|v| v.contains("raw hex color")));
    assert!(token_check.violations.iter().any(|v| v.contains("raw size")));
    ...
    assert!(kit_check.violations.iter().any(|v| v.contains("aria-modal")));
}
